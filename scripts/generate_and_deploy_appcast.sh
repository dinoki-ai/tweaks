...
curl -L -f -o "updates/arm64/Tweaks-${VERSION}.dmg" \
  "https://github.com/${PUBLIC_REPO}/releases/download/${VERSION}/Tweaks-${VERSION}.dmg"

if [ ! -f "updates/arm64/Tweaks-${VERSION}.html" ]; then
...
template = f"""<!doctype html><html><head><meta charset=\"utf-8\"><title>Tweaks {version} Release Notes</title>
...
<h1>Tweaks {version}</h1>
{body_html}
</body></html>"""
pathlib.Path(f'updates/arm64/Tweaks-{version}.html').write_text(template, encoding='utf-8')
PY
fi
...
if ! grep -q 'edSignature' updates/appcast-arm64.xml; then
  ...
  SIG_OUTPUT=$(./sparkle_tools/bin/sign_update --ed-key-file private_key.txt "updates/arm64/Tweaks-${VERSION}.dmg" | tr -d '\n') || true
  ...
  if ! grep -q 'edSignature' updates/appcast-arm64.xml; then
    tmpfile=$(mktemp)
    sed -E "s#(<enclosure[^>]*Tweaks-${VERSION}\.dmg\"[^>]*)[[:space:]]*/>#\\1 sparkle:edSignature=\"${EDSIG}\"/>#g" updates/appcast-arm64.xml > "$tmpfile"
    mv "$tmpfile" updates/appcast-arm64.xml
  fi
...
tmpfile=$(mktemp)
sed "s/Tweaks-${VERSION}\.html/Tweaks-latest.html/g" updates/appcast.xml > "$tmpfile"
...
tmpfile=$(mktemp)
sed -E "s#<sparkle:releaseNotesLink>[^<]*Tweaks-latest\.html</sparkle:releaseNotesLink>#<sparkle:releaseNotesLink>${PAGES_BASE}Tweaks-latest.html</sparkle:releaseNotesLink>#g" updates/appcast.xml > "$tmpfile"
mv "$tmpfile" updates/appcast.xml
...
cp "updates/arm64/Tweaks-${VERSION}.html" "public-repo/docs/Tweaks-latest.html"
cd public-repo
git config user.name "github-actions[bot]"
git config user.email "github-actions[bot]@users.noreply.github.com"
git add docs/appcast.xml \
  docs/Tweaks-latest.html
