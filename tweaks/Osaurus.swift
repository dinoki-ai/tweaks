...
final class Osaurus {
    ...
    init(baseURL: URL? = nil, session: URLSession = .shared) throws {
        ...
        } else if let override = ProcessInfo.processInfo.environment["OSAURUS_BASE_URL"], let url = URL(string: override) {
            self.baseURL = url
            #if DEBUG
    ...
    final class Chat {
        ...
        final class Completions {
            ...
            func create(model: String, messages: [OsaurusChatMessage], temperature: Double? = nil) async throws -> OsaurusChatCompletionResponse {
                ...
                let payload = OsaurusChatCompletionRequest(model: model, messages: messages, temperature: temperature)
                ...
                guard (200...299).contains(http.statusCode) else { throw OsaurusError.httpError(http.statusCode) }
...
extension Osaurus {
    enum Defaults {
        ...
        static let systemPrompt: String = "You are Osaurus Tweak. Improve the user's copied text for clarity and tone. Preserve meaning and formatting. Output only the revised text without any preface."
        static let temperature: Double = 0.3
    }

    func tweak(text: String,
               model: String = Defaults.model,
               systemPrompt: String = Defaults.systemPrompt,
               temperature: Double = Defaults.temperature) async throws -> String {
        let messages = [
            OsaurusChatMessage(role: "system", content: systemPrompt),
            OsaurusChatMessage(role: "user", content: text)
        ]
        let response = try await chat.completions.create(model: model, messages: messages, temperature: temperature)
