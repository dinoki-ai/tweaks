...
@MainActor class PermissionManager: ObservableObject {
  ...
  private func startMonitoring() {
    ...
    let timer = Timer(
      timeInterval: 5.0, target: self, selector: #selector(handleNormalTimer(_:)), userInfo: nil,
      repeats: true)
  ...
  private func startIntensiveMonitoring() {
    checkTimer?.invalidate()
    let timer = Timer(
      timeInterval: 0.5, target: self, selector: #selector(handleIntensiveTimer(_:)), userInfo: nil,
      repeats: true)
    RunLoop.main.add(timer, forMode: .common)
    checkTimer = timer
  }
      ...
      if showingHelp {
        VStack(alignment: .leading, spacing: 4) {
          Text("Why is this needed?")
            ...
            .fontWeight(.medium)
          Text(
            "Accessibility permission allows the app to simulate keyboard shortcuts for pasting text."
          )
          .font(.caption)
          .foregroundColor(.secondary)
